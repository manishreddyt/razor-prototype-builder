  ...
  return (
    <DashboardLayout>
      <div className="p-3 sm:p-6 max-w-6xl mx-auto space-y-6 sm:space-y-8">
        {/* Hero Header */}
        <div className="rounded-xl sm:rounded-2xl border bg-gradient-to-br from-primary/5 via-background to-secondary/30 p-4 sm:p-8">
          <div className="max-w-2xl">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-foreground tracking-tight">App Marketplace</h1>
          ...
          {/* Search */}
          <div className="relative max-w-md mt-4 sm:mt-5">
            ...
            <Input
              ...
              className="pl-9 sm:pl-10 h-9 sm:h-11 rounded-lg sm:rounded-xl bg-background/80 backdrop-blur-sm border-border/60 text-sm"
            />
          </div>
        </div>
        ...
        <div className="flex gap-1.5 sm:gap-2 flex-wrap">
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-[10px] sm:text-xs font-medium transition-all border ${
                activeCategory === cat
        ...
        {activeCategory === "All" && !search && featuredApps.length > 0 && (
          <div>
            <h2 className="text-xs sm:text-sm font-semibold text-foreground mb-2 sm:mb-3 flex items-center gap-1.5 sm:gap-2">
              <TrendingUp className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-primary" /> Featured Apps
            </h2>
              ...
              {featuredApps.map((app) => {
                ...
                return (
                  <button
                    ...
                    className="group relative flex gap-3 sm:gap-4 rounded-xl sm:rounded-2xl border bg-card p-3 sm:p-5 text-left transition-all hover:shadow-lg hover:border-primary/30 hover:-translate-y-0.5"
                  >
                    <div className="h-11 w-11 sm:h-14 sm:w-14 rounded-lg sm:rounded-xl overflow-hidden shrink-0 border bg-background">
                      {app.logo ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                        <h3 className="font-bold text-sm sm:text-base text-foreground">{app.name}</h3>
                        {isInstalled && <Badge variant="secondary" className="text-[9px] sm:text-[10px]">Installed</Badge>}
                        <Badge className="text-[9px] sm:text-[10px] bg-primary/10 text-primary border-0">Featured</Badge>
                      </div>
                      <p className="text-[11px] sm:text-xs text-muted-foreground mt-0.5 sm:mt-1 leading-relaxed line-clamp-2">{app.description}</p>
        ...
        <div>
          {activeCategory === "All" && !search && (
            <h2 className="text-xs sm:text-sm font-semibold text-foreground mb-2 sm:mb-3">All Apps</h2>
          )}
            ...
            {filtered.map((app) => {
              ...
              return (
                <button
                  key={app.id}
                  onClick={() => navigate(`/app-marketplace/${app.id}`)}
                  className="group relative flex flex-col rounded-lg sm:rounded-xl border bg-card text-left transition-all hover:shadow-md hover:border-primary/20 hover:-translate-y-0.5 overflow-hidden"
                >
                  {/* Card top bar */}
                  <div className="flex items-start gap-2.5 sm:gap-3.5 p-3 sm:p-5 pb-2 sm:pb-3">
                    <div className="h-9 w-9 sm:h-11 sm:w-11 rounded-lg sm:rounded-xl overflow-hidden shrink-0 border bg-background">
                      {app.logo ? (
                        <img src={app.logo} alt={app.name} className="h-full w-full object-cover" />
                      ) : (
                        <div className={`h-full w-full flex items-center justify-center ${app.iconBg}`}>
                  ...
                  {/* Description */}
                  <p className="text-[11px] sm:text-xs text-muted-foreground leading-relaxed px-3 sm:px-5 pb-2 sm:pb-3 flex-1 line-clamp-2">
                    {app.description}
                  </p>

                  {/* Footer */}
                  <div className="flex items-center justify-between px-3 sm:px-5 py-2 sm:py-3 border-t bg-muted/30">
