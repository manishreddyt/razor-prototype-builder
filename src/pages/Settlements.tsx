  ...
  <DashboardLayout>
    <div className="animate-fade-in space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Track settlement cycles from Razorpay to your bank</p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-secondary/50">
              <th className="blade-table-header px-5 py-3 text-left">Settlement ID</th>
              <th className="blade-table-header px-5 py-3 text-left">Amount</th>
              <th className="blade-table-header px-5 py-3 text-left">UTR</th>
              <th className="blade-table-header px-5 py-3 text-left">Transactions</th>
          ...
          <tbody>
            {settlements.map((s) => (
              <tr key={s.id} className="border-b border-border last:border-0 hover:bg-secondary/30 transition-colors">
                <td className="px-5 py-3 font-medium text-primary">{s.id}</td>
                <td className="px-5 py-3 text-foreground font-medium">{s.amount}</td>
                <td className="px-5 py-3 text-muted-foreground">{s.utr}</td>
                <td className="px-5 py-3 text-foreground">{s.txnCount}</td>
                <td className="px-5 py-3 text-muted-foreground">{s.date}</td>
                <td className="px-5 py-3"><span className={statusClass[s.status] || "blade-badge"}>{s.status}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
